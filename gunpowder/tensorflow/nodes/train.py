...
class Train(GenericTrain):
    ...
    Args:
        ...
        reference (``dict``, :class:``ArrayKey`` -> :class:``ArraySpec``):

            Dictionary from ArrayKeys to ArraySpecs used to calculate necessary
            upstream dependencies based on the requested outputs.
