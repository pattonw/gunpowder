...
class SyntheticLightLike(BatchProvider):
    ...
    def provide(self, request: BatchRequest) -> Batch:
        ...
        for _ in range(self.n_obj):
            for _ in range(100):
                ...
                for i, connected_component in enumerate(
                    nx.weakly_connected_components(tree)
                ):
                    for point in connected_component:
                        tree.nodes[point]["component"] = i
        ...
        for i, connected_component in enumerate(
            nx.weakly_connected_components(trees_graph)
        ):
            for point in connected_component:
                trees_graph.nodes[point]["component"] = i
        self._plot_tree(trees_graph)
    ...
    def _grow_tree(self, root: np.ndarray, roi: Roi) -> nx.DiGraph:
        ...
        while len(leaves) > 0 and len(tree.nodes) < self.num_nodes[1]:
            ...
            if len(tail) == 1:
                tail.append(np.random.random(tail[0].shape) + tail[0])
    ...
    def _plot_tree(self, tree):
        ...
        node_adjacencies = []
        node_text = []
        for node_id, node_attrs in tree.nodes.items():
            node_adjacencies.append(node_attrs["component"])
            node_text.append("component: " + str(node_attrs["component"]))
