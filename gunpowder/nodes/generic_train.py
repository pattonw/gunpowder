...
class GenericTrain(BatchFilter):
    ...
    Args:
        ...
        relative_shapes (dict): Dictionary from the names of input and output
            :class:``ArrayKey``'s to :class:``Coordinate`` containing relative
            sizes, Allowing Train node's to compute upstream requests from
            requested outputs.
    ...
    def __init__(
        ...
        for key in itertools.chain(
            self.outputs.values(), self.gradients.values(), self.inputs.values()
        ):
            assert key in reference, f"Key {key} has no shape infomation!"
    ...
    def prepare(self, request):
        ...
        offset = None
        for key in self.provided_arrays:
            if self.reference[key].nonspatial or key not in request:
                continue
            reference_roi = self.reference[key].roi
            request_roi = request[key].roi

            assert reference_roi.get_shape() == request_roi.get_shape(), (
                f"Requested shape {request_roi.get_shape()} does not match "
                f"expected shape {reference_roi.get_shape()} from reference"
            )

            if offset is None:
                offset = request_roi.get_begin() - reference_roi.get_begin()
            else:
                key_offset = request_roi.get_begin() - reference_roi.get_begin()
                assert offset == key_offset, (
                    f"No single offset can shift the reference request roi's "
                    f"onto the requested output rois"
                )
        for key in self.inputs.values():
            input_spec = self.reference[key]
            if offset is not None:
                input_spec.roi.shift(offset)
            deps[key] = input_spec
        return deps
