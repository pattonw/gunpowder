...
class TestElasticAugmentGraph(ProviderTest):
    def test_3d_basics(self):
        ...
        for _ in range(5):
            ...
            with build(pipeline):
                ...
                self.assertTrue(0 in points.data, msg="{}".format(points.data))
                ...
                for i, point in points.data.items():
                    ...
                    loc = Coordinate(loc / labels.spec.voxel_size)
                    if labels_data_roi.contains(loc):
                        self.assertEqual(
                            labels.data[loc], i, msg="{}".format(points.data)
                        )

    @unittest.skipIf(
        not hasattr(BatchRequest, "random_seed"), "random seed feature not installed!"
    )
